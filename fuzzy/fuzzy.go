...
/*
Package fuzzy implements Alfred-like fuzzy search, e.g. "of"
matches "OmniFocus" and "got" matches "Game of Thrones".
...
// Interface makes a slice fuzzy-sortable.
// The standard sort.Interface (i.e. Less()) is used as a fallback for
// data with the same score.
//
// TODO: Possible to simplify to a single method plus an intermediate wrapper struct?
...
// Sort fuzzy-sorts data based on query and returns the scores.
// Highest scores/best matches first.
//
// Sort *does not* truncate data. To only show matching results,
// you must check scores:
//
//    for i, score := range workflow.Sort(data, query) {
//        if score == 0.0 {
//            data = data[:i]
//            break
//        }
//    }
//
func Sort(data Interface, query string) []float64 {

	fq := newFuzzyQuery(query, data)
	sort.Sort(fq)
	return fq.scores
...
func (fq fuzzyQuery) Len() int { return fq.Data.Len() }
